 ...
 */
public class ZrElement extends AbstractZElement implements Streamable {

/**
	 * 
	 */
	private static final long serialVersionUID = 5557961668558699420L;
...
//        this.value = value.multiply(BigIntegerUtils.TWO).mod(order);
    ...
    public ZrElement setToRandom() {
        ...
        /*if ((this.value.bitLength()>order.bitLength())&&(this.value.compareTo(order)!=1)){
        	throw new IllegalArgumentException("Bitlength ("+this.value.bitLength()+") too long!!!");
        }*/
        if (this.value.bitLength()>order.bitLength()){
        	//temporary fix for GWT issue
        	this.value = new BigInteger(this.value.toString(16),16);
        }
    ...
    public ZrElement setFromHash(byte[] source, int offset, int length) {
        ...
        for (;;) {
            ...
            Arraycopy.arraycopy_custom(source, offset, buf, i, n);
            i += n;
    ...
    public byte[] toBytes() {
        ...
        if (bytes.length > field.getLengthInBytes()) {
            ...
            if (bytes[0] == 0 && bytes.length == field.getLengthInBytes() + 1) {
                ...
                throw new IllegalStateException("ZrElement.toBytes: result("
                     +bytes.length+", "+value+", "+value.bitLength()
                     +") has more than FixedLengthInBytes("
                     +field.getLengthInBytes()
                     +").");
            }
        } else if (bytes.length < field.getLengthInBytes()) {
            byte[] result = new byte[field.getLengthInBytes()];
            Arraycopy.arraycopy_custom(bytes, 0, result, field.getLengthInBytes() - bytes.length, bytes.length);
            return result;
        }
        return bytes;
    }
